...
export const Dashboard: React.FC = () => {
    ...
    const handleUploadLogs = async () => {
        ...
        try {
            const result = await uploadLogs(selectedMachine, selectedFile, false);
            if (result.message && result.message.includes('processed 0 logs')) {
                setUploadStatus('Данные не найдены');
                alert('В файле не найдены подходящие данные для анализа (проверьте названия колонок s1-s4 или sensor_1-sensor_4)');
            } else {
            ...
            let detailedError = 'Ошибка загрузки';
            try {
                // If it's a fetch error, we might need to check if the response was JSON
                // However, our uploadLogs service throws Error if !response.ok
                // We might need to modify api.ts to include the body in the error
                detailedError = err.message || 'Ошибка загрузки';
            } catch (e) {
                detailedError = 'Ошибка загрузки';
            }
            setUploadStatus(detailedError);
        } finally {
            setIsCreating(false);
        }
    };
        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center">
                ...
                <p className="text-white/50 font-mono text-sm tracking-widest animate-pulse uppercase">
                    {error ? <span className="text-warning">{error}</span> : 'Загрузка конфигурации...'}
                </p>
                {error && (
                    <button 
                                ...
                                <div key={m.id} className="group relative flex items-center gap-2">
                                    ...
                                    {m.owner_id && (
                                        <button 
                                            ...
                                            className="p-3 text-white/20 hover:text-warning hover:bg-warning/10 rounded-lg transition-all opacity-0 group-hover:opacity-100 border border-transparent hover:border_WARNING/20"
                                        >
                        ...
                        >
                            ...
                            {latestLog ? (
                                ...
                                <div className="text-center py-8 text-white/30 animate-pulse font-mono text-xs uppercase flex flex-col gap-2">
                                    Нет данных: загрузите логи для анализа
                                </div>
                            )}
                        </motion.div>
