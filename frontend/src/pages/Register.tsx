...
import { Activity, UserPlus, ArrowRight, CheckCircle } from 'lucide-react';
...
export const Register: React.FC = () => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // После успешной регистрации сразу пробуем выполнить вход,
            // чтобы получить реальный токен из бэкенда
            ...
            Cookies.set('username', loginData.username, { expires: 1 });

            setSuccess(true);
            setTimeout(() => navigate('/'), 1500);
        } catch (err: any) {
            setError(err.message === 'Username already registered' ? 'Имя пользователя уже занято' : err.message);
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="w-full max-w-md">
                ...
                <div className="glass-panel p-8 relative overflow-hidden">
                    ...
                    {success ? (
                        <div className="text-center py-8">
                            <CheckCircle className="w-16 h-16 text-electric mx-auto mb-4" />
                            <h2 className="text-xl font-bold text-white mb-2">Регистрация успешна</h2>
                            <p className="text-white/60 font-mono text-sm">Инициализация протокола дашборда...</p>
                        </div>
                    ) : (
                ...
                <div className="text-center mt-6">
                    <button
                        ...
                        className="mt-3 text-white/60 hover:text-white font-mono text-sm flex items-center gap-2 mx-auto transition-colors group"
                    >
                        Уже есть аккаунт? Войти <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                    </button>
                </div>
