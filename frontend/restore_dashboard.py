...
export const Dashboard: React.FC = () => {
    ...
    const handleAddMachine = async () => {
        ...
        try {
            ...
            if (selectedFile) {
                setUploadStatus('Загрузка логов...');
                await uploadLogs(newMachineName, selectedFile, false);
            }
    ...
    const handleCreateKey = async (mode: 'dataset' | 'analysis') => {
        await generateApiKey(mode);
        loadData();
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 flex flex-col gap-6">
                    ...
                    >
                        ...
                        >
                            <Activity className="w-3 h-3" /> Снять показания
                        </button>
                    </motion.div>
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-3">
                                ...
                                <div key={k.id} className="bg-white/5 border border-white/5 rounded-lg p-3 flex flex-col gap-2 group">
                                    <div className="flex justify-between items-center">
                                        <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${
                                            k.mode === 'dataset' ? 'bg-[#FF00E5]/10 border-[#FF00E5]/30 text-[#FF00E5]' : 'bg-electric/10 border-electric/30 text-electric'
                                        }`}>
                                            {k.mode.toUpperCase()}
                                        </span>
                                        <button 
                            ...
                            <button 
                                onClick={() => handleCreateKey('analysis')}
                                className="w-full py-2 border border-dashed border-white/10 rounded-lg text-[10px] font-mono text-white/20 hover:text-white/40 transition-colors mt-2"
                            >
                                + Создать ключ анализа
                            </button>
                        </div>
                    </motion.div>
                </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Log Upload */}
                                <div>
                                    <label className="text-[10px] font-mono text-white/30 uppercase tracking-widest mb-2 block">История логов (Архив .zip / .h5)</label>
                                        ...
                                        >
                                            {selectedFile ? (
                                                <>
                                                    ...
                                                    <span className="text-xs font-mono truncate max-w-[200px]">{selectedFile.name}</span>
                                                </>
                                            ) : (
                                                <>
                                                    <Upload className="w-8 h-8 opacity-50" />
                                                    <span className="text-xs font-mono">Перетащите или кликните</span>
                                                </>
                                            )}
                                        </label>
                                    </div>
                                ...
                                <div className="bg-electric/5 border border-electric/10 rounded-xl p-4 flex gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-xs font-bold text-electric mb-1 uppercase tracking-tight">API Защита</h4>
                                        <p className="text-[10px] text-white/40 leading-relaxed font-mono">
                                            Новый станок будет доступен для автоматической подачи данных, используя сгенерированный API ключ в вашем кабинете.
                                        </p>
                                    </div>
                                </div>
...
print("Successfully restored and updated Dashboard.tsx")
